...
| 资金流向 | `northbound.py`（北向跨境）、`a_share_fund_flow.py`（境内 A 股可查表/排名）、`capital_flow.py`（个股摘要） |
| 基础 / 财务 | `financials.py` |
| 工具 | `utils/`、`tick/` |
...
## 附录 H：A 股境内资金流向（`tool_fetch_a_share_fund_flow`）

| `query_kind` | AkShare 主路径（实现以代码为准） | 说明 |
|--------------|----------------------------------|------|
| `market_history` | `stock_market_fund_flow` | 沪深两市大盘资金流历史，`max_days` 内存截取 |
| `sector_rank` | `stock_sector_fund_flow_rank`；备：`stock_fund_flow_industry` / `stock_fund_flow_concept` | `sector_type`→东财 `sector_type`；地域链仅东财 |
| `stock_rank` | `stock_fund_flow_individual`；备：`stock_individual_fund_flow_rank` | `rank_window` 映射 THS `symbol` / 东财 `indicator` |
| `stock_history` | `stock_individual_fund_flow` | 单股多日表 |
| `big_deal` | `stock_fund_flow_big_deal` | 可选按代码列过滤 |
| `main_force_rank` | `stock_main_fund_flow` | `main_force_symbol` 与 AkShare 枚举一致 |
| `sector_drill` | `stock_sector_fund_flow_summary`；可选 `stock_sector_fund_flow_hist` / `stock_concept_fund_flow_hist` | `sector_name` 须与东财列表一致 |

官方索引：[AkShare 股票数据 · 资金流向](https://akshare.akfamily.xyz/data/stock/stock.html)。签名与列探测笔记：[AkShare_fund_flow_probe_notes.md](./AkShare_fund_flow_probe_notes.md)。

补充：`utils/eastmoney_fund_flow_direct.py` 使用与 AkShare 相同的东财 push2 / push2his URL，配合 `request_with_retry` 与 **clist 行数上限**，作为 AkShare 全量分页前的优先路径（大盘、个股历史、主力排名）。

## 附录 I：A 股技术选股排名表（`tool_fetch_a_share_technical_screener`）

同花顺数据中心技术选股，AkShare `stock_rank_*_ths`；与 [AkShare 文档 · A 股 · 技术指标](https://akshare.akfamily.xyz/data/stock/stock.html) 目录下创新高/连续上涨/量价齐升等条目对应。

| `screener_kind` | AkShare 函数 | `variant` |
|-----------------|--------------|-----------|
| `new_high` | `stock_rank_cxg_ths` | `month` / `half_year` / `year` / `all_time` → 创月/半年/一年/历史 **新高** |
| `new_low` | `stock_rank_cxd_ths` | 同上 → **新低** |
| `continuous_up` | `stock_rank_lxsz_ths` | — |
| `continuous_down` | `stock_rank_lxxd_ths` | — |
| `volume_expansion` | `stock_rank_cxfl_ths` | — |
| `volume_contraction` | `stock_rank_cxsl_ths` | — |
| `breakout_up` | `stock_rank_xstp_ths` | `ma5`…`ma500` → `N日均线` |
| `breakout_down` | `stock_rank_xxtp_ths` | 同上 |
| `price_volume_up` | `stock_rank_ljqs_ths` | — |
| `price_volume_down` | `stock_rank_ljqd_ths` | — |
| `insurance_holding_raise` | `stock_rank_xzjp_ths` | — |

超时：`AKSHARE_TECH_SCREENER_TIMEOUT_SEC`，否则回退 `AKSHARE_FUND_FLOW_ATTEMPT_TIMEOUT_SEC`（默认 180s）。**非** K 线本地指标：请用 `tool_stock_data_fetcher` / `tool_calculate_technical_indicators`。
